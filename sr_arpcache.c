...
void handle_arpreq(struct sr_instance* sr, struct sr_arpreq* request)
{
    ...
    {
	/*
         * if(times_sent < 5), then create an ARP request for the next hop
	 * ip address
         */
	if(request->times_sent < 5)
	{
            fprintf(stderr, "times_sent < 5\n");
            uint8_t* packet = (uint8_t*)malloc(sizeof(sr_ethernet_hdr_t) + 
                                               sizeof(sr_arp_hdr_t));
	    ...
	    struct sr_rt* table_walker = sr->routing_table;
            fprintf(stderr, "when times_sent < 5, do LPM\n");
	    while(table_walker)
	    {
		uint32_t match_check = table_walker->mask.s_addr & request->ip;
	    ...
	    memcpy(arp_hdr->ar_sha, selected_interface->addr, ETHER_ADDR_LEN);
	    arp_hdr->ar_sip = selected_interface->ip;
	...
	else
	{
	    ...
	    while(packet_walker)
	    {
		struct sr_if* curr_interface = sr_get_interface(sr, packet_walker->iface);
		handle_icmp_unreach(sr, packet_walker->buf, 0x03, 
				    0x01, curr_interface);
